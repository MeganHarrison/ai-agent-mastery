...
@app.post("/api/langgraph-sequential-agents")
async def langgraph_routing_agents_endpoint(
    request: AgentRequest, 
    user: Dict[str, Any] = Depends(verify_token)
):
    """
    LangGraph sequential agent workflow endpoint with streaming and conversation history.
    Sequential flow for research -> data enrichment -> outreach (email draft)
